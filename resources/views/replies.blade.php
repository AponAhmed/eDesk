...
@section('content')
    <div id="mails" class="flex flex-col h-full ox-h w-full bg-white">
        <div class="list-wraper ox-h">
            ...
            @else
                @foreach ($messages as $message)
                    <div data-id="{{ $message->id }}" class="mail-list replies-list mail-list-item ox-h hover:bg-slate-50">
                        <div class="list-inner ox-h flex items-center relative">
                            ...
                            <div class="name-wraper flex flex-col md:flex-row md:items-center ox-h w-full">
                                <a href="javascript:void(0)"
                                    class="form-name md:px-2">{{ $message->getOptions()['toName'] }}</a>
                                <div class="list-content ox-h">
                                    <div class="message-control absolute right-1 top-0 flex">
                                        <div class="leading-none text-sm px-2 pb-[1px] font-mono bg-gray-100 text-gray-700 rounded-full border border-solid border-gray-200"
                                            title="Replied at {{ $message->date() }}">
                                            @php echo $message->date() @endphp
                                        </div>
                                    </div>
                                    <a href="javascript:void(0)" class="subject-line">{{ $message->subject }}</a>
        ...
        <!-- Pagination Links -->
        <div
            class="action-pagination flex items-center py-1 px-2 justify-between bg-gray-50 border border-t-0 border-solid border-gray-200">
            <div class="action-wrap flex px-2 items-center">
                ...
                <div class="dropdown relative inline-block text-left">
                    <div>
                        <button title="Action"
                            class="dropdown-toggler rounded-full flex flex-col items-center justify-between h-7 w-7 p-2 bg-white border border-solid border-gray-300">
                            <span class="bg-gray-700 h-[2px] w-[2px] rounded-full"></span>
                            <span class="bg-gray-700 h-[2px] w-[2px] rounded-full"></span>
                            <span class="bg-gray-700 h-[2px] w-[2px] rounded-full"></span>
                        </button>
                    </div>
                    <div class="dropdown-content absolute left-0 bottom-full z-10 mb-2 w-56  rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                        role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                        <div class="py-1" role="none">
                            @foreach ($actions as $indx => $action)
                                <a href="javascript:void(0)" data-action-title="{{ $action->label }}"
                                    data-action="{{ $action->action }}"
                                    class="multiple-action-trigger text-gray-700 block px-4 py-2 text-sm hover:bg-slate-100"
    ...
    <div id="messageDetails" class="mail-details h-full bg-white"></div>
@endsection
